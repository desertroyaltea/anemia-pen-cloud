...
# -*- coding: utf-8 -*-
"""
extract_features.py (fast, glare-aware, cached, parallel)

Speedups with zero accuracy change:
- Caches Roboflow detections keyed by image SHA1 + model id
- Parallelizes glare mask + inpainting + feature extraction
- Keeps the SAME feature formulas as before (no resizing, no math changes)

Pipeline:
1) Read full-eye images from --images-dir
2) Roboflow detect API to crop conjunctiva -> --out-crops
   * CACHED: detection JSON saved under --cache-dir
3) Detect glare, inpaint, compute features on inpainted crop:
   - 14 baseline features
   - 6 vascularity (Frangi + skeleton)
   - glare_frac
4) Join with labels.csv (filename,hb,status) -> --out-csv

Optional:
--augment-glare N  (off by default; adds synthetic glare variants)
...
def parse_args():
    ...
    p.add_argument("--sleep", type=float, default=0.0, help="DEPRECATED; use --rf-rate-limit")
    p.add_argument("--rf-rate-limit", type=float, default=0.0, help="Seconds to wait between Roboflow calls (0 = no delay)")
    p.add_argument("--timeout", type=float, default=60.0, help="Roboflow HTTP timeout seconds")
    p.add_argument("--workers", default="max", help="'max' or integer number of worker processes for feature extraction")
    p.add_argument("--cache-dir", default=".cache", help="Directory for caching Roboflow detections and features")
    p.add_argument("--no-cache", action="store_true", help="Disable all caching (always hit API / recompute features)")

    # Vessel / morphology params (unchanged math)
    ...
    # Optional augmentation (off by default to avoid extra work)
    p.add_argument("--augment-glare", type=int, default=0,
                   help="Per image, generate N synthetic glare variants (then mask+inpaint+extract); off by default")
    return p.parse_args()
...
def roboflow_detect_b64(b64_str: str, model_id: str, api_key: str, conf_0_100: int, timeout: float) -> Dict[str, Any]:
...
# -------- Glare handling (unchanged formulas) -------- #
...
def add_synthetic_glare(rgb: np.ndarray, n_spots: int = 1) -> np.ndarray:
    """Overlay bright, soft-edged ellipses to simulate glare (for augmentation)."""
    h, w, _ = rgb.shape
    aug = rgb.copy()
...
# -------- Feature extractors (unchanged math) -------- #
...
def vascularity_features_from_conjunctiva(rgb_u8: np.ndarray,
    ...
    NumPy 2.0-safe normalization included.
    """
    ...
    vmap = (vmap - vmap.min()) / (np.ptp(vmap) + 1e-8)  # NumPy 2.0-safe
...
# ------------- Core (parallel) ------------- #
def process_one_crop_bytes(crop_bytes: bytes,
                           filename_tag: str,
                           black_ridges: bool,
                           min_size: int,
                           area_threshold: int) -> Dict[str, float]:
    """Worker-safe: operate from bytes to avoid cross-process PIL handles."""
    ...
    vas_feats = vascularity_features_from_conjunctiva(
        rgb_proc, black_ridges=black_ridges,
        min_size=min_size, area_threshold=area_threshold
    )
...
def main():
    ...
    rf_cache_dir = cache_root / "roboflow" / args.model_id.replace("/", "_")
    feat_cache_dir = cache_root / "features"
    ...
    # --- PHASE 1: detection + crop (sequential, but cached) ---
    # Build a list of (crop_bytes, filename_tag) to feed into the parallel phase.
    tasks: List[Tuple[bytes, str]] = []

    for i, img_path in enumerate(files, 1):
        try:
            pil_full = exif_upright(Image.open(img_path))
            # Compute file hash for caching
            fhash = sha1_of_file(img_path)
            rf_cache_file = rf_cache_dir / f"{fhash}.json"

            # Get detection (cache first)
            if not args.no_cache and rf_cache_file.exists():
                with open(rf_cache_file, "r", encoding="utf-8") as f:
                    rf_json = json.load(f)
            else:
                b64 = to_b64_jpeg(pil_full)
                rf_json = roboflow_detect_b64(
                    b64, model_id=args.model_id, api_key=args.api_key,
                    conf_0_100=int(args.conf), timeout=args.timeout
                )
                if not args.no_cache:
                    with open(rf_cache_file, "w", encoding="utf-8") as f:
                        json.dump(rf_json, f)
                # Respect gentle rate limit if provided
                if args.rf_rate_limit > 0:
                    time.sleep(args.rf_rate_limit)
            ...
            # Buffer bytes for parallel feature extraction
            crop_bytes = pil_to_jpeg_bytes(crop, quality=95)

            # (A) Original crop
            tasks.append((crop_bytes, img_path.name))
            ...
            for j in range(max(0, int(args.augment_glare))):
                ...
                tasks.append((pil_to_jpeg_bytes(aug_img, quality=95), tag))

            print(f"[{i}/{len(files)}] {img_path.name}: OK (crop{f', +{args.augment_glare} aug' if args.augment_glare>0 else ''})")
    ...
    # --- PHASE 2: features (parallel, cached per filename tag) ---
    def feature_cache_key(filename_tag: str) -> Path:
        # Cache key contains filename tag; if you change vessel params, include them here to keep caches separate
        key = hashlib.sha1( (filename_tag + f"|br={args.black_ridges}|ms={args.min_size}|at={args.area_threshold}").encode("utf-8") ).hexdigest()
        return feat_cache_dir / f"{key}.json"

    rows: List[Dict[str, float]] = []

    # Determine workers
    if str(args.workers).lower() == "max":
        n_workers = os.cpu_count() or 4
    else:
        n_workers = max(1, int(args.workers))

    # Submit parallel jobs; reuse cache when available
    to_compute: List[Tuple[bytes, str]] = []
    for crop_bytes, tag in tasks:
        if not args.no_cache and feat_cache_dir.exists():
            fcache = feature_cache_key(tag)
            if fcache.exists():
                try:
                    with open(fcache, "r", encoding="utf-8") as f:
                        rows.append(json.load(f))
                    continue
                except Exception:
                    pass
        to_compute.append((crop_bytes, tag))

    if to_compute:
        with ProcessPoolExecutor(max_workers=n_workers) as ex:
            futs = [
                ex.submit(
                    process_one_crop_bytes, cb, tag,
                    args.black_ridges, args.min_size, args.area_threshold
                )
                for (cb, tag) in to_compute
            ]
            for fut in as_completed(futs):
                ...
                # write feature cache
                if not args.no_cache:
                    fcache = feature_cache_key(row["filename"])
                    try:
                        with open(fcache, "w", encoding="utf-8") as f:
                            json.dump(row, f)
                    except Exception:
                        pass
    ...
    # For zero-risk default, we keep 'inner' join (aug rows drop unless labeled explicitly).
    merged = labels_df.merge(feat_df, on="filename", how="inner")

    out_csv = Path(args.out_csv)
    merged.to_csv(out_csv, index=False)
    print(f"\nSaved features CSV: {out_csv.resolve()}")
    print(f"Cropped images at: {out_dir.resolve()}")
    if not args.no_cache:
        print(f"Cache directory: {cache_root.resolve()}")
        print("Tip: next runs will skip API calls and reused features when files are unchanged.")
